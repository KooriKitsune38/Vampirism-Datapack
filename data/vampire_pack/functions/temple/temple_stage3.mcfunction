#> vampire_pack:temple/temple_stage3
...
# Summons
#> Armor Stand
#summon armor_stand ~ ~-1 ~ {NoGravity:1b,Invulnerable:1b,Small:1b,Invisible:1b,Tags:["ArmorStandParticles"]}
#> Villager
execute unless entity @e[tag=Anchor,type=villager,distance=..5] align xyz run summon villager ~0.5 ~-.9 ~0.5 {id:"minecraft:villager",Tags:[Anchor,TempleStage3],CustomName:'{"text":"Vampire Temple"}',Silent:1b,Invulnerable:1b,NoAI:1b,ActiveEffects:[{Id:14b,Amplifier:1b,Duration:9999999,ShowParticles:0b}],VillagerData:{profession:"minecraft:none"},Offers:{Recipes:[{maxUses:1,buy:{id:"minecraft:nether_star",Count:1b},sell:{id:"minecraft:paper",Count:1b,tag:{display:{Name:'{"text":"Final Sacrifice"}',Lore:['{"text":"Show that you can defeat the immortal creatures"}','{"text":"to embrace the darkness and leave the mortal world."}']},StageActivator:4}}}]}}

## Effects ##
tellraw @a[distance=..150] {"text":"The Darkness has ascended","color":"#BF6363","bold":true}
effect give @a[distance=..150,predicate=!vampire_pack:player/tag_vampire] minecraft:blindness 20 0

## Advancements ##
execute as @a[distance=..7] run advancement grant @s only vampire_pack:temple/infernal_demands