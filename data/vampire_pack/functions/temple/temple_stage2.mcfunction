#> vampire_pack:temple/temple_stage2
...
# Summons
#> Armor Stand
#summon armor_stand ~ ~ ~ {NoGravity:1b,Invulnerable:1b,Small:1b,Invisible:1b,Tags:["ArmorStandParticles"]}
#> Anchor
execute unless entity @e[tag=Anchor,type=villager,distance=..5] align xyz run summon villager ~0.5 ~.1 ~0.5 {id:"minecraft:villager",Tags:[Anchor,TempleStage2],CustomName:'{"text":"Vampire Temple"}',Silent:1b,Invulnerable:1b,NoAI:1b,ActiveEffects:[{Id:14b,Amplifier:1b,Duration:9999999,ShowParticles:0b}],VillagerData:{profession:"minecraft:none"},Offers:{Recipes:[{maxUses:1,buy:{id:"minecraft:gold_block",Count:4b},buyB:{id:"minecraft:wither_skeleton_skull",Count:4b},sell:{id:"minecraft:paper",Count:1b,tag:{display:{Name:'{"text":"Infernal Demands"}',Lore:['{"text":"Embrace the darkness and"}','{"text":"listen to your deepest nightmares"}']},StageActivator:3}}}]}}

# Effects ##
tellraw @a[distance=..100] {"text":"The Corners Of Fire have risen from the darkness.","color":"#BF6363","bold":true}
effect give @a[distance=..100,predicate=vampire_pack:player/tag_vampire] minecraft:blindness 10 0

# Scoreboard
#scoreboard players set @e[tag=Anchor,distance=..3] tempcurrentstage 2

# Advancement ##
advancement grant @a[distance=..7] only vampire_pack:temple/corners_of_fire